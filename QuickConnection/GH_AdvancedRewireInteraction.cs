            ...
            else  if (target != null)
            {
                if (target != source)
                {
                    ...
                    if ((bool)_inputInfo.GetValue(this))
                    {
                        ...
                        list.AddRange(document.UndoUtil.CreateWireEvent("rewire_source", source).Actions);
                        list.AddRange(document.UndoUtil.CreateWireEvent("rewire_target", target).Actions);
                    ...
                    {
                        List<IGH_UndoAction> list2 = new List<IGH_UndoAction>();
